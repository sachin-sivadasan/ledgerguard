...
| RPT-UNINSTALL-1 ✅ | `/reports/uninstall-context` | ~~all "Unknown", tenure 0~~ **FIXED (PR pending):** two bugs — (1) correlation used a GID-only index but uninstall events store the myshopify DOMAIN → every paying shop missed → "Unknown"; now correlates by any identifier (GID/domain/name). (2) tenure measured from `CreatedAt` (rebuild ≈ now → floored to 0); now from `StartDate()` (ActivatedAt). Backend-only, no resync. | ✅ |
