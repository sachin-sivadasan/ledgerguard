...
| RPT-FEES-2 ✅ | tier config | The Fee Guard caught a REAL mismatch: app configured `SMALL_DEV_0` (0%) but Shopify actually retains **~15%**. **FIXED (PR pending):** `/fees/monthly` now **derives** the real rate from the observed `shopifyFee/gross`, snapped to the nearest Shopify tier (0/15/20% via `snapToTierRate`), and the Fee Guard compares each month against that DETECTED rate — so consistent months pass and only true anomalies (e.g. the Jun 0%→15% transition month) flag, instead of every month. Response adds `detected_fee_pct` + `configured_fee_pct` + `tier_matches` to surface the config mismatch. **Follow-up:** frontend banner showing "configured 0% but Shopify retains 15% — update your tier" (needs service→provider plumbing; logged). | ✅ |
...
| RPT-PAYOUT-1 | `/reports/payout-schedule`, `/payout-history` | **INVESTIGATED — mostly data-availability, not a bug.** The `-3399` first row is a legit refund (`AppSaleAdjustment`, negative net). **payout-history empty** = the Partner API does NOT expose real payouts; `EarningsStatus=PAID_OUT` is a local prediction that's never actually set, so nothing lands in history (verdict (c): no payout data source — needs a Shopify Payouts feed we don't have). **payout-schedule `nextPayoutDate` in the past** = it picks the *earliest* available date (oldest, past) instead of the next *future* one — a real but minor cosmetic fix, DEFERRED (the fix is time-relative and the report's ~15 tests hard-code dates against `time.Now()`; not worth destabilizing for a data-limited report). → future.md. | 🟡 |
| RPT-REVIEWS-1 ✅ VERIFIED | `/reports/reviews` | **RESOLVED (live): set slug=zoko → 49 reviews, avg 5.0.** Config, not a bug — Reviews come from a web scraper (`ShopifyAppStoreClient`, `apps.shopify.com/{slug}/reviews`), NOT the Partner API. `ReviewProcessor` is wired into Wave 1 but **gated on `app.AppStoreSlug`**, which is **empty** for this app (`"app_store_slug":""`) → sync skips → 0 reviews. Fix = set the app's App Store slug (e.g. Zoko → its store slug); then the scraper runs. **The internal API already exists** (`PATCH /api/v1/apps/{appID}/store-slug`, Firebase auth) — now **hardened** (PR pending): validates the slug format + normalizes a pasted `apps.shopify.com/<slug>` URL down to the slug. Set it, then reviews populate on the next sync. → future.md. | 🟡 |
