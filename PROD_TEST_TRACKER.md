...
**APPS-1b 🟡 (roadmap) install analytics + revenue lens.** Now that we capture the full install lifecycle: (a) surface installs/uninstalls **trend** + lifecycle tiles (installed/uninstalled/reactivated/active) in the existing Installs report (Mantle-like); (b) **install→paid conversion** (charged shops ÷ total installs ≈ 24%) — the revenue angle Mantle buries; (c) install-cohort **revenue retention**. Also: `status_sync` could read the now-complete stored `app_events` instead of re-fetching per-shop (kills ~2,930 API calls). See future.md.
