...
| RPT-REVIEWS-1 | `/reports/reviews` | **INVESTIGATED — config, not a bug.** Reviews come from a web scraper (`ShopifyAppStoreClient`, `apps.shopify.com/{slug}/reviews`), NOT the Partner API. `ReviewProcessor` is wired into Wave 1 but **gated on `app.AppStoreSlug`**, which is **empty** for this app (`"app_store_slug":""`) → sync skips → 0 reviews. Fix = set the app's App Store slug (e.g. Zoko → its store slug); then the scraper runs. Config action, not code. → future.md. | 🟡 |
