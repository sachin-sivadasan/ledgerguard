...
func TestFetchTransactions_AppCredit(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Return mock response with AppCredit (uses "amount" instead of "netAmount")
		response := map[string]interface{}{
			"data": map[string]interface{}{
				"transactions": map[string]interface{}{
					"edges": []map[string]interface{}{
						{
							"cursor": "cursor1",
							"node": map[string]interface{}{
								"id":        "gid://partners/AppCredit/12345",
								"createdAt": "2024-02-15T10:00:00Z",
								"app": map[string]interface{}{
									"id":   "gid://partners/App/99",
									"name": "Test App",
	...
	// AppCredit should be classified as REFUND
	if transactions[0].ChargeType != valueobject.ChargeTypeRefund {
		t.Errorf("expected charge type REFUND for AppCredit, got %s", transactions[0].ChargeType)
	}
	if transactions[0].AmountCents != 1500 {
		t.Errorf("expected amount 1500 cents, got %d", transactions[0].AmountCents)
	}
}
