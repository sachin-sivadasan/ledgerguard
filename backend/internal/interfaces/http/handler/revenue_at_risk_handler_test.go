...
// TestRevenueAtRisk_Paging verifies the ranked at-risk stores table is paged server-side:
// no paging returns every store with storesTotal = full count; a limit/offset window
// returns just that slice (MRR-desc order preserved); KPIs ignore paging; and an offset
// past the end yields an empty window with total unchanged.
func TestRevenueAtRisk_Paging(t *testing.T) {
	appID := uuid.New()
	pa := &entity.PartnerAccount{ID: uuid.New(), UserID: uuid.New()}
	app := &entity.App{ID: appID, PartnerAccountID: pa.ID, Name: "Test App"}

	// 6 one-cycle-missed stores, MRR increasing with index → sorted desc, shop 'f' leads.
	const n = 6
	subs := make([]*entity.Subscription, 0, n)
	for i := 0; i < n; i++ {
		domain := string(rune('a'+i)) + ".myshopify.com"
		subs = append(subs, atRiskSub(appID, domain, "Pro", int64(1000*(i+1)), valueobject.RiskStateOneCycleMissed))
	}
	h := NewRevenueAtRiskHandler(
		&mockSubscriptionRepo{subscriptions: subs},
		&mockSnapshotRepoForForecast{},
	...
	decode := func(query string) revenueAtRiskReport {
		t.Helper()
		req := newRevenueAtRiskRequest(t, appID, pa, query)
		rec := httptest.NewRecorder()
		h.GetRevenueAtRisk(rec, req)
		if rec.Code != http.StatusOK {
			t.Fatalf("expected 200, got %d: %s", rec.Code, rec.Body.String())
	...
	wantTotal := all.TotalAtRiskCents

	// limit=2&offset=1 → the [1,3) window (MRR desc: f,e,d,c,b,a → offset 1 = e).
	...
	if page.TotalAtRiskCents != wantTotal {
		t.Errorf("paged totalAtRiskCents = %d, want %d (KPIs must ignore paging)", page.TotalAtRiskCents, wantTotal)
	}
