		...
		if cfg.AppHandler != nil && cfg.AuthMW != nil {
			r.Route("/apps", func(r chi.Router) {
				...
				if cfg.MetricsHandler != nil {
					...
					r.Get("/{appID}/metrics/trend", cfg.MetricsHandler.GetMetricsTrend)
				...
				// Churn report route
				if cfg.ChurnHandler != nil {
					r.Get("/{appID}/reports/churn", cfg.ChurnHandler.GetChurn)
				}
				...
				// MRR report route
				if cfg.MRRReportHandler != nil {
					r.Get("/{appID}/reports/mrr", cfg.MRRReportHandler.GetMRRReport)
				}

				// Active Customers report route
				if cfg.ActiveCustomersReportHandler != nil {
					r.Get("/{appID}/reports/active-customers", cfg.ActiveCustomersReportHandler.GetActiveCustomersReport)
				}
				...
				// Subscriptions (ARPU / LTV) report route (Archetype B — composition)
				if cfg.SubscriptionsReportHandler != nil {
					r.Get("/{appID}/reports/subscriptions", cfg.SubscriptionsReportHandler.GetSubscriptions)
				}
