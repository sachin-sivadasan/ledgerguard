...
| 🛡️ **Ledger Reconciliation** | ✅ **Shipped** — Guard. `ledger_reconciliation_report_handler.go` (GET `/reports/ledger-reconciliation`): per-month **itemized decomposition** — `gross = net + revenue_share + processing`, where processing is Shopify's payment fee derived per sale at sync (`gross − shopifyFee − net`, since the Partner API exposes no processing/tax field). Residual = what those buckets don't explain (isolates refund/credit rows whose fee reversal never synced); flags months with a residual > 1%. CSV, 401. `frontend-flutter/.../ledger_recon_screen.dart` (reconciled/flagged verdict + gross/net/revenue-share/processing KPIs + per-month table). Depends on the derive-processing-fee + `AppSaleCredit` sync change. |
