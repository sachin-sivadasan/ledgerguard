...
| **Activation** | ✅ **Shipped** — `backend/.../activation_report_handler.go` (11 tests; Growth · Archetype E funnel. Three NESTED install-cohort stages joining app-events ↔ subs: Installs (RELATIONSHIP_INSTALLED shops in-window) ⊇ Started (also a SUBSCRIPTION_CHARGE_ACCEPTED event — the "approved a plan" signal, sourced from events NOT the subs table so it stays distinct from Paid) ⊇ Paid (sub reached a recurring charge, `LastRecurringChargeDate` set); shop identity canonicalised GID↔domain so webhook/sync event keys collapse; overall/install→sub/sub→paid pcts div-by-zero-guarded; 503×2, 401, CSV) + `frontend-flutter/.../activation_screen.dart` (3 conversion KPIs + narrowing funnel bars + end-to-end footer). **NOTE: app-events are ingested per-subscription + dev-capped at 10 (see future.md), so the funnel renders sparse/empty with live data until event coverage widens.** |
