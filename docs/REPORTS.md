...
| **Net-New Subscriptions** | ✅ **Shipped** — `backend/.../net_new_subs_report_handler.go` (11 tests; Growth Archetype A. New (subs CreatedAt in range) vs churned (churnedDateOf in range, reuses churn helper) + net; start-and-churn-same-period counts both; daily new/churned/net trend; recent-new-subs table [newest-first, capped 50, UI "N of M", plan/MRR/started]; currency first-non-empty break-loop [no sentinel collision]; date boundary; 503, 401, CSV) + `frontend-flutter/.../net_new_subs_screen.dart` (New/Churned/Net KPIs, net-new bar chart, new-subs table). Merged to main (PR #TBD). |
