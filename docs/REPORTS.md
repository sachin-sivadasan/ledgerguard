...
| 🛡️ **Ledger Reconciliation** | ✅ **Shipped** — Guard. `ledger_reconciliation_report_handler.go` (GET `/reports/ledger-reconciliation`): per-month money-identity check — recorded net vs Shopify's gross − fee; flags months with a residual (usually incomplete fee data). CSV, 401. `frontend-flutter/.../ledger_recon_screen.dart` (reconciled/flagged verdict + gross/fee/net KPIs + per-month table with ✓/⚠ residual). |
