...
| Activation | install → paid conversion | ✅ **Shipped** (PR) — install/accepted events ↔ subs funnel (sparse until event coverage widens) |
...
| Customer Insights (Segments) | slice customers by app-plan / risk / revenue-band | ✅ | subs + `risk_state` |
...
| Ledger Reconciliation | does the money add up? (gross = net + share + processing) | ✅ | `fee_verification_service` |
...
| **Installs** | ✅ **Shipped** — `backend/.../installs_report_handler.go` (Growth Archetype A. Install/uninstall counts + net from RELATIONSHIP_INSTALLED/UNINSTALLED app-events [UNINSTALL matched FIRST — "UNINSTALLED" contains "INSTALL"]; daily two-series trend; recent-events table [newest-first, capped 50, domain resolved via sub ShopifyShopGID join]; date-window boundary; 503×2, 401, CSV) + `frontend-flutter/.../installs_screen.dart` (Installs/Uninstalls/Net KPIs, two-line trend, events table with pills). Merged to main (PR #20). **APPS-1b:** added an all-time **lifecycle tile row** (Active / Installed / Uninstalled / Reactivated) from `external.CountLifecycle` (single-sourced with the persisted install count) + an **install→paid conversion** headline (distinct paying shops ÷ lifetime installs) linking to the Activation funnel. |
...
| **Activation** | ✅ **Shipped** — `backend/.../activation_report_handler.go` (Growth. **2-stage all-time Installs → Paid funnel** after RPT-ACTIVATION-1: the old 3-stage design keyed its middle "Started" stage on `SUBSCRIPTION_CHARGE_ACCEPTED` events, which the live Partner stream barely emits (~120 shops), throttling paid to 40 vs the real ~2,931. Now reuses the Installs report's `computeLifecycleAndConversion` helper — Installs (de-fragmented lifetime install base) → Paid (distinct shops with a recurring charge) — so the funnel and the Installs page's conversion headline are **identical by construction**; div-by-zero + >100% guarded; 503×2, 401, CSV) + `frontend-flutter/.../activation_screen.dart` (Install→Paid % + Installs/Paying-stores KPIs + 2-bar funnel). |
| **Customer Insights** | ✅ **Shipped** — Customers · Archetype B (segmentation). `backend/.../customer_insights_report_handler.go` (6 tests; slices the current customer base three cross-cutting ways the single-dimension reports don't: **revenue bands** (MRR buckets over the active base), **risk segments** (SAFE/AT_RISK/CHURNED across the whole base), and a **plan × risk crosstab** (where at-risk revenue concentrates) + **top customers by MRR**; active base = non-churned; 401, 503, CSV) + `frontend-flutter/.../customer_insights_screen.dart` (4 KPIs + revenue-band bars + plan-risk table + top-customers list with risk pills). **NOTE:** the catalog's "Shopify plan" cut is intentionally dropped — `transactions.shop_plan` is never populated (the Partner API's `Shop` object carries only id/domain/name/avatar, no merchant plan; that tier lives only in each merchant's Admin API, which we can't reach). The *app* plan (PlanName) — the actionable one — is the crosstab. **Plan labels:** since the Partner API gives no plan name either (`ledger_service.go:244`), `plan_label.go` derives a price-tier label (`planKey` = interval + base price; `planLabeler`: real name → developer map (Phase 2) → pseudo-label `"$139.99/mo"`). Shared across Customer Insights, Active Customers, Subscriptions, and Net-New so plan groupings agree. **Tail collapse:** `BasePriceCents` is the last *charged* amount, so prorations + refund/adjustment (negative-price) subs spawn a long tail of 1–2 customer phantom tiers (Zoko: ~68 tiers, 7 real). Past `minTiersToCollapse` (12) tiers, ones below `tierSignificanceThreshold` (max 3, 0.5% of active base) fold into a trailing **"Other (N tiers)"** row — but a **named** tier never folds (`isPseudoPlanLabel`). Settings hides the same minor tiers (keeps named) + returns a `hiddenTiers` count. Root cause (prorated/negative BasePriceCents at the ledger level) logged P2 in `future.md`. |
| 🛡️ **Fee Audit** | ✅ **Shipped** — the Guard differentiator. `backend/.../fee_audit_report_handler.go` (GET `/reports/fee-audit`) reuses the shared `buildFeeAudit` engine (also powers Profit & Expense) — derives the app's real revenue-share rate from `shopifyFee` (RPT-FEES-2), flags months whose actual cut deviates from it, and surfaces the configured-vs-actual tier mismatch + savings vs the 20% default; months param, CSV, 401. `frontend-flutter/.../fee_audit_screen.dart` (verdict banner + tier-mismatch note + KPIs [effective rate / total cut / saved-vs-20%] + per-month audit table with ✓/⚠ variance). |
| 🛡️ **Ledger Reconciliation** | ✅ **Shipped & prod-verified** (PR #77) — Guard. `ledger_reconciliation_report_handler.go` (GET `/reports/ledger-reconciliation`): per-month **itemized decomposition** — `gross = net + revenue_share + processing`, where processing is Shopify's payment fee derived per sale at sync (`gross − shopifyFee − net`, since the Partner API exposes no processing/tax field — see [[shopify-partner-api-gotchas]]). Residual = what those buckets don't explain (isolates refund/credit rows whose fee reversal never synced), flagged at >1% of gross. **Silent-absorption guard:** a month whose derived processing rate exceeds a plausible ceiling (`processingSuspectPct = 6%`; real processing ~3%, tiers 15/20%) is marked `processing_suspect` and does NOT reconcile even at zero residual — stops a silently-missing `shopifyFee` (its cut absorbed into processing) from reading as clean. CSV, 401. `frontend-flutter/.../ledger_recon_screen.dart` (verdict + gross/net/revenue-share/processing KPIs + per-month table showing processing rate + "fee unsynced" vs residual status). Depends on the derive-processing-fee + `AppSaleCredit` sync change. **Live (Zoko, 6 mo): 6/6 reconciled, processing steady ~3.1%, residual −0.016%.** |
| 🛡️ Payout Accuracy | ⬜ Pending (Guard) — data-blocked (no Partner payout feed) |
