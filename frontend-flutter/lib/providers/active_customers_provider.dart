...
class ActiveCustomersProvider extends ChangeNotifier {
  final ActiveCustomersService _service;
  ...
  ActiveCustomersReport? _report;

  ActiveCustomersProvider(this._service);
  ...
  ActiveCustomersReport? get report => _report;
  ...
  ActiveCustomersReport _mockReport() {
    final now = DateTime.now();
    // Trend rising to 148 across 8 points.
    const trendValues = [140, 141, 143, 144, 145, 146, 147, 148];
    return ActiveCustomersReport(
      ...
      trend: List.generate(
        trendValues.length,
        (i) => ActiveTrendPoint(
          date: now.subtract(Duration(days: (trendValues.length - 1 - i) * 4)),
          activeCustomers: trendValues[i],
        ),
      ),
      plans: [
        ActivePlan(
