...
class RetentionProvider extends ChangeNotifier {
  ...
  RetentionReport? _report;

  RetentionProvider(this._service);
  ...
  RetentionReport? get report => _report;
  ...
  RetentionReport _mockReport() {
    final now = DateTime.now();
    return RetentionReport(
      currency: 'USD',
      renewalRate: 0.92,
      retainedMrrCents: 1136000,
      reactivations: 3,
      trend: List.generate(
        8,
        (i) => RetentionTrendPoint(
          date: now.subtract(Duration(days: (7 - i) * 4)),
          // Gentle upward-ish drift with a little noise.
          renewalRate: 0.88 + i * 0.005 + (i.isEven ? 0.002 : -0.002),
        ),
      ),
      plans: [
        RetentionPlan(
          ...
          activeSubs: 84,
          renewalRate: 0.94,
          retainedMrrCents: 686000,
        ),
        RetentionPlan(
          planName: 'Growth',
          activeSubs: 52,
          renewalRate: 0.91,
          retainedMrrCents: 338000,
        ),
        RetentionPlan(
          planName: 'Starter',
          activeSubs: 37,
          renewalRate: 0.89,
          retainedMrrCents: 112000,
        ),
      ],
    );
  }
