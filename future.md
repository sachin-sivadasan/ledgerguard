...
The Activation report (Growth · Archetype E, shipped) joins RELATIONSHIP_INSTALLED + SUBSCRIPTION_CHARGE_ACCEPTED app-events ↔ subscriptions into an install→started→paid funnel. It is CORRECT but renders **sparse/empty with live data** because of two limits in `internal/infrastructure/queue/processors/event_processor.go`:
- **Per-subscription event fetch** (`:63-85`): the processor iterates existing subscriptions to get shop GIDs, so it only fetches events for shops that already have a subscription. Installs by stores that never subscribed (the top of the funnel) are only captured via real-time webhooks, not backfill — so the install universe is under-counted historically.
- **`DEV LIMIT` caps event fetch to the first 10 subscriptions** (`:68-72`, "revert for production") — in the current deployment events exist for ≤10 shops.
- **Fix:** remove the dev-10 cap; drive event fetch from the install-event/store universe (or a store list) rather than only existing subscriptions, so non-converting installs are captured. Then the funnel's Installs and Started stages reflect reality. Same event pipeline also feeds the Installs report and Uninstall Context.
