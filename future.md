...
| Install analytics + revenue lens | P2 | Now that `event_sync` ingests the **app-wide** relationship-event stream (all shops), build on it: (a) **Installs report** trend + lifecycle tiles (Installed / Uninstalled / Deactivated / Reactivated / Active) — a focused version of Mantle's install page (don't clone it; install charts are commoditized/Shopify-free); (b) **install→paid conversion** (charged shops ÷ total installs, ≈24% for Zoko) — the revenue angle Mantle buries and our differentiator; (c) **install-cohort revenue retention** ("which installs became paying and stayed"). Also **efficiency**: `status_sync` still re-fetches events per-shop (~2,930 Partner-API calls) — switch it to read the now-complete stored `app_events` (reorder so event_sync completes first), eliminating the redundant fetch. Context: APPS-1 in `PROD_TEST_TRACKER.md`. |
