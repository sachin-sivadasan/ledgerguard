...
| Reviews report — set App Store slug (RPT-REVIEWS-1) | P3 | `/reports/reviews` is empty because reviews come from a **web scraper** (`apps.shopify.com/{slug}/reviews`), not the Partner API, and `ReviewProcessor` is gated on `app.AppStoreSlug` which is empty for the live app. Fix = populate `app_store_slug` (Zoko → its store slug) so `review_sync` runs — a config/data action. Optionally auto-derive the slug from the app name or let the user set it in the UI. |
| Derive/verify revenue-share tier (Fee Guard accuracy) | P2 | `App.RevenueShareTier` defaults to `SMALL_DEV_0` (0%) for every app, but live data shows Zoko actually pays **~15%** (crossed $1M lifetime → moved to the 15% tier; RPT-FEES-2). So the Fee Guard's expected baseline is wrong and it flags every month for mis-tiered apps. Fix options: (a) **derive** the effective tier from the observed `shopifyFee/gross` ratio (it lands on exactly 0/15/20%) and use that as the baseline — the Guard then only flags true anomalies; (b) fetch the real tier from the Partner API if exposed; (c) let the user set it. Also handle **tier transitions** (Jun showed 9.7% = part-0%/part-15% as the $1M threshold was crossed mid-month) — a lifetime-earnings-aware expected rate. Context: RPT-FEES-2 in `PROD_TEST_TRACKER.md`. |
