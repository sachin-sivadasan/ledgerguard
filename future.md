...
| Reports — wireframe-gap follow-ups | P3 | Deferred from the date-filter/polish PR: (1) **Revenue at Risk recoverable trend line** — the wireframe (19a) shows a 2nd dashed-green "recoverable" series in the trend, but `RevenueAtRiskTrendPoint` only carries `atRiskCents` (no per-point recoverable). Needs a backend change to emit a recoverable value per trend point, then a 2nd LineChartBarData. (2) **Interval-aware chart x-axis labels** — backend now emits `interval` (day/week/month) on the installs/net-new/mrr/churn/retention/usage trend responses and month buckets are dated first-of-month; the Flutter trend charts still format every label as `MMM d`. Parse `interval` in those services and format month buckets as `MMM yyyy` (day/week stay `MMM d`). (3) **Adaptive granularity for usage-trends** was intentionally SKIPPED — it's a weekly WoW-momentum report by design. |
...
| Reports — Payout History PAID_OUT not populated | P2 | **The pipeline never assigns `EarningsStatusPaidOut`** — `EarningsCalculator.DetermineEarningsStatus` only ever returns PENDING/AVAILABLE, and `UpdateEarningsStatus` preserves but never transitions *into* PAID_OUT. So the Payout History report (PR #19) is **effectively always empty with live data** until a path sets PAID_OUT (e.g. syncing Shopify's real payout/disbursement events, or ageing AVAILABLE→PAID_OUT past the disbursement window). Also the `EarningsStatusPaidOut` entity comment says "Earnings have been disbursed" — aspirational, not currently true. The report code is correct and will light up once PAID_OUT is populated. **Recommended approach (time-based estimate — the Partner API does NOT expose per-charge payout/disbursement events, only sales `netAmount`, so real dates aren't syncable):** add a third time-derived state to `EarningsCalculator.DetermineEarningsStatus` — PENDING (`now < availableDate`), AVAILABLE (`availableDate ≤ now < paidOutDate`), PAID_OUT (`now ≥ paidOutDate`) where `paidOutDate = availableDate + PayoutDelayDays` (new constant; ~23d ≈ charge+30d monthly cycle is a sensible default, tune 14–30). Deterministic + idempotent (fits rebuild-every-sync); also lights up the Earnings report's Paid-Out KPI. Label the data "estimated" in the UI. Rule #6 (alters earnings/rebuild logic) → confirm before implementing. |
| Reports — Payout History real payout events | P3 | Deferred from the Payout History build (PR #19): rows are grouped by **charge calendar month** (a proxy), and the "available date" column is EarningsCalculator's ~7-day estimate — neither is a real Shopify payout/disbursement event (we don't sync those yet). When Shopify's actual payout batches/dates are available, group by real payout event and show the true disbursement date. Also (silent-failure): a period whose paid charges all lack an availability date renders "—" with no per-period log; and a PAID_OUT tx with no charge date buckets under 0001-01 (now logged in aggregate). |
| Reports — Payout Schedule real dates | P3 | Deferred from the Payout Schedule build (PR #18): the timeline's available dates are LedgerGuard-computed (`EarningsCalculator.CalculateAvailableDate` = charge date + a flat **7-day** delay in MVP; Shopify actually clears in 7–37 days), NOT Shopify's authoritative payout dates. The report caption and handler doc now say "estimated ~7 days", but a user reconciling against real Shopify payouts will see mismatches. When we sync Shopify's actual payout/disbursement dates, use them for the schedule. Also: header-only CSV for an empty schedule downloads with no "no data" signal (frontend `bytes.isEmpty` is false for a header row) — minor UX. |
| Reports — Subscriptions LTV precision | P3 | Deferred from the Subscriptions (ARPU/LTV) build (PR): (1) **Per-plan LTV uses the app-level churn rate** — we don't have reliable per-plan churn, so every plan's LTV divides its own ARPU by the single blended churn rate; a real Starter-vs-Enterprise churn gap is invisible. Compute per-plan churn once churn can be attributed by plan. (2) **Churn lens = logo churn only** — LTV uses `churnedSubs ÷ snapshotTotal` (subscription/logo churn), not revenue churn; document the lens choice on the KPI and consider a revenue-churn LTV variant (see [[mantle-brain-reference]] 3-lens churn). (3) **Churn denominator is the latest snapshot's TotalSubscriptions** (a point-in-time count), so a stale/behind snapshot inflates the rate (clamped to 1.0, logged) — same live-numerator ÷ snapshot-denominator staleness the Churn report guards. (4) **ARPU floors** whole cents via integer division (documented); acceptable, but a fractional-cent ARPU is invisible. |
| Transactions — loadMore swallows page errors | P3 | Deferred from the Transactions server-side-totals build (PR #45): `TransactionProvider.loadMore` catches a failed page fetch and only `debugPrint`s it, leaving `_error` untouched — so an infinite-scroll page that fails to load looks identical to end-of-data (the list silently stops growing). Pre-existing; contradicts the deliberate rethrow added to `fetchTransactions` in the same PR. Set `_error` (or a non-fatal load-more flag) so a failed page is distinguishable from reaching the end. |
| Transactions — store filter is client-only / page-scoped | P3 | Deferred from PR #45: `_storeFilter` is applied client-side in the `transactions` getter and is NOT sent to the server (`TransactionFilters` has no store/domain field for the list/summary queries). In live mode it therefore only narrows the ~20 loaded rows, not the full dataset, and the header count still reflects all stores. The PR mitigated the totals mismatch (server totals fall back to page-fold when a store filter is active) but the underlying filter is still incomplete. Proper fix: thread store/domain into `TransactionFilters` + both the paginated list and `GetTransactionSummary` queries + `fetchSummary`, so store filtering (and its count/totals) is server-side and full-dataset. |
| Install analytics — cohort retention + sync efficiency | P2 | Partial: (a) lifecycle tiles + (b) install→paid conversion headline **shipped** on the Installs report (APPS-1b, PR pending) — tiles from `external.CountLifecycle`, conversion links to the existing Activation funnel. Remaining: (c) **install-cohort revenue retention** ("which installs became paying and stayed") — group by *install* month (distinct from the shipped signup-cohort Retention report), track paid-conversion + retained revenue per cohort over time; new backend aggregation + a cohort chart. Also **efficiency**: `status_sync` still re-fetches events per-shop (~2,930 Partner-API calls) — switch it to read the now-complete stored `app_events` (reorder so event_sync completes first), eliminating the redundant fetch. Context: APPS-1/APPS-1b in `PROD_TEST_TRACKER.md`. |
| Structured logging (zap / slog) | P3 | The whole backend uses **stdlib `log.Printf`** (52 files; `[queue]`/`[partner]`/`[API]` string prefixes). Migrate to a structured logger — prefer stdlib **`log/slog`** (no new dep) or `go.uber.org/zap` — for level control, JSON output, and queryable fields (app_id, job_id, wave, shop_gid) instead of prefix-grepping. Cross-cutting: introduce a logger in `main.go`, thread via context or a package logger, replace `log.Printf` call-sites incrementally (start with the sync processors, which already carry job_id/app_id). Do as its OWN PR series — do not mix into feature work. |
...
| Item | Priority | Notes |
|------|----------|-------|
| Mobile Ratings & Reviews — cache + full metric set | P3 | Shipped (PR #85) as **fetch-on-demand, links-only** (Apple lookup+reviews, Google JSON-LD rating). Two follow-ups: (1) **Cache** the public-store fetches (short TTL per app, e.g. 15 min, or a daily snapshot) — every screen load currently hits Apple + Google live; Google's HTML scrape is fragile and unofficial, so a cache both cuts load and shields against transient upstream/parse breakage (log a distinct "parse-drift vs network" error so markup changes are detectable). (2) The **credentialed** path (App Store Connect `.p8` + Play service account) for **downloads / revenue / crashes** — the metrics that are NOT public — if a real audience of multi-platform devs materializes. |
| App-ownership check missing in resolveAppFromRequest (IDOR, codebase-wide) | P2 | `resolveAppFromRequest` (`app_lookup.go`) does a bare PK `apps` lookup and never checks `app.PartnerAccountID` against the caller's partner account, so any authenticated user who knows an app UUID can read/write its data (mobile links, reports, plan labels, slug, etc.). Pre-existing across ALL app-scoped handlers (net-new subs, revenue-at-risk, plan labels, …), not introduced by any one feature. Fix once, centrally: scope `FindByID` by partner account, or assert ownership in `resolveAppFromRequest`. Needs a careful audit of every call site + tests. |
| Dashboard "This Week Activity" — wire to real events | P3 | The dashboard activity widget (installs/uninstalls/churns/upgrades counts) only has mock data; live it returns `{}` and now shows a neutral "No activity data" empty state. To populate it, the DashboardProvider needs to fetch app events (EventsService) and aggregate by type over the selected window — same event source the Installs report uses. Low priority (overlaps the Installs/Events pages); the empty state is honest in the meantime. |
| Subscription BasePriceCents = plan price, not last prorated/negative charge | P2 | The ledger rebuild sets `BasePriceCents` from the last recurring charge, which for a mid-cycle upgrade is a **prorated partial** ($77.48) and for refund/adjustment subs is **negative**. Result: price tiers fragment into a long tail of phantom "plans" (Zoko: ~68 tiers, 7 real; see the display-side "Other (N tiers)" collapse in `plan_label.go`/report handlers, already shipped). Proper fix = derive the recurring price from the **modal / most-recent FULL (non-prorated) recurring charge** and never from a negative adjustment, so subscriptions land on their real plan price and far fewer phantom tiers exist. Touches `ledger_service.go` sub construction + needs a resync. Display collapse stays as a safety net. |
| Populate subscription PlanName (app plan name) — lights up plan segmentation across 3 reports | P2 | **Every subscription has an empty `PlanName`** because the ledger rebuild builds subs from transactions and hardcodes `"" // Plan name not available from transactions` (`ledger_service.go:244`). Confirmed via live schema: the Partner API `AppSubscriptionSale` carries only `app/billingInterval/chargeId/createdAt/grossAmount/netAmount/shop/shopifyFee` — **no plan name**, just the `chargeId` (AppSubscription GID) + `billingInterval` (ANNUAL / EVERY_30_DAYS). Live proof (Zoko): Active Customers + Customer Insights both return a single `planName:""` bucket for all 1,103 active subs. **Impact:** the plan dimension is blank in Active Customers ("by plan"), Subscriptions (ARPU/LTV by plan), and Customer Insights (plan × risk crosstab) — 3 reports. **Fix options:** (a) query the `AppSubscription` charge object per unique `chargeId` GID to get the developer-set plan name (new Partner API query + a chargeId→name cache; verify the AppSubscription object actually exposes a name field first), or (b) derive a pseudo-label from `billingInterval` + `BasePriceCents` (e.g. "$139.99/mo") as a cheap proxy — not the real plan name but restores a usable segmentation, or (c) let the developer upload a plan-GID→name map. Near-term UX: render empty plan names as "Unspecified" and collapse the crosstab to a note when there's only one unnamed plan, so the live reports don't show a blank row. |
| Customer Insights — segment by merchant's Shopify plan (Admin API) | P3 | The Customer Insights report (shipped #78) intentionally **omits the "Shopify plan" cut** (Basic/Shopify/Advanced/Plus) — `transactions.shop_plan` is never populated because the **Partner API's `Shop` object exposes only `id/myshopifyDomain/name/avatarUrl`, no plan**. The merchant's Shopify tier lives ONLY in each store's **Admin API** (`shop { plan { displayName partnerDevelopment shopifyPlus } }` / `GET /admin/.../shop.json` → `plan_display_name`), which needs a **per-shop OAuth access token** minted at install — NOT the Partner/app API key (that key is only an app *identity* used to start OAuth, not a data credential). LedgerGuard reads the *developer's Partner account* and isn't installed in merchant stores, so it holds no Admin tokens. **Only realistic path:** if LedgerGuard is allowed to tap the developer's OWN app's per-shop tokens (the app the merchants installed), add a separate Admin-API sync that calls `shop.plan` per store (~2.7k calls for Zoko, refreshed as plans change) and populates `shop_plan`; then group the report by it. Large new pipeline + privacy-sensitive token access for a low-value dimension ("how many of my customers run Plus") — hence deferred. The *app* plan (PlanName: Starter/Growth/Pro) — the actionable one — is already the report's plan × risk crosstab. |
| Payout reports — real payout feed + next-date fix (RPT-PAYOUT-1) | P3 | Two issues, both low-value: (1) `/payout-history` is always empty because Shopify's Partner API exposes **no payout data** — `EarningsStatus=PAID_OUT` is a local prediction that's never set. A real history needs a Shopify Payouts source (webhook/import) we don't have; until then, relabel the report as "estimated" or hide it. (2) `/payout-schedule` `nextPayoutDate` shows the *earliest* available date (often in the past) instead of the next *future* one — fix = earliest `AvailableDate >= today` (I have a working patch); deferred only because the handler's ~15 tests hard-code dates against `time.Now()` and need converting to a fixed injected `now` first. Do the test-determinism refactor + the 1-line fix together. |
| Reviews report — App Store slug in the UI (RPT-REVIEWS-1) | P3 | ✅ **Core resolved** — slug set via `PATCH /apps/{id}/store-slug` (hardened #70), sync-clobber fixed (#72); live: `zoko` → 49 reviews, avg 5.0. Remaining polish: surface the slug field in a **settings UI** (so it's not a curl-only action), and optionally auto-suggest the slug from the app name. |
| Lifetime-threshold-aware Fee Guard (revenue-share tier) | P2 | **The reduced-share plan is a lifetime $1M THRESHOLD, not a static tier.** `SMALL_DEV_0` (0% on the first $1M lifetime) and `SMALL_DEV_15` (15% after $1M) are the SAME plan at two stages — so a single static `App.RevenueShareTier` can't be correct across an app's history. Live proof (Zoko): 0% early → **Jun 9.7%** (the crossing month — part 0%, part 15%) → **Jul/Aug 15.0%**. Current state: #68's Fee Guard already sidesteps the static tier for the per-month `fee_guard_ok` (it compares vs the DETECTED rate), so only `tier_matches`/`configured_fee_pct` (cosmetic) are affected. **Proper fix:** compute the "expected" rate as **lifetime-earnings-aware** — 0% on cumulative earnings ≤ $1M, 15% beyond, allowing a split WITHIN the crossing month (reproduces 0/9.7/15 exactly). The Guard then only flags a true miss (a month jumping to 20%, or a rate going *backwards* after crossing). **Caveat:** the $1M threshold is on lifetime earnings from program start, but our sync only reaches back to `SyncHistoryStart` — if the app passed $1M before our earliest synced tx we can't reconstruct the crossing from data alone; infer it from the observed rate-step (visible in Jun), or let the user enter lifetime-earnings-to-date once. Model the tier as "reduced plan (0%→15% @ $1M)" vs "default 20%", not two independent tiers. Context: RPT-FEES-2 in `PROD_TEST_TRACKER.md`. |
...
| Independent processing-fee verification + refund reconciliation (Ledger Reconciliation) | P3 | ✅ **Core shipped & prod-verified** (PR #77): sync derives `ProcessingFeeCents = gross − shopifyFee − net` per sale + fetches `AppSaleCredit`, and the Ledger Reconciliation report reconciles `gross = net + revenue_share + processing` (Zoko live: 6/6 months, processing ~3.1%, residual −0.016%). **Known limitation:** processing is a **derived plug**, not independently reported — Shopify's Partner API exposes only `grossAmount`/`netAmount`/`shopifyFee` per sale (no processing/tax field; `grossAmount` excludes tax), so the identity closes by construction and can't *verify* the processing charge, only surface it. The only real signals it catches today are (a) `processing_suspect` (rate >6% ⇒ `shopifyFee` didn't sync, its cut absorbed into processing) and (b) refund/credit rows whose fee reversal never synced (a residual). **Future:** reconcile refunds/credits (`AppSaleAdjustment`/`AppSaleCredit`) as their own line — decompose the residual into "unsynced refund fees" explicitly rather than lumping — and, if a Shopify Payouts feed ever exists (see RPT-PAYOUT-1), cross-check the derived processing against actual disbursements for a *true* zero-verification instead of a self-closing identity. |
...
## App-event ingestion coverage — widen for the Activation funnel (P2)
The Activation report (Growth · Archetype E, shipped) joins RELATIONSHIP_INSTALLED + SUBSCRIPTION_CHARGE_ACCEPTED app-events ↔ subscriptions into an install→started→paid funnel. The **dev-10-cap is now removed (ADR-046)** — events are fetched for all subscriptions. **One structural limit remains** in `internal/infrastructure/queue/processors/event_processor.go`:
- **Per-subscription event fetch** (`:62-85`): the processor iterates existing subscriptions to get shop GIDs, so it only fetches events for shops that **already have a subscription**. Installs by stores that never subscribed (the top of the funnel) are only captured via real-time webhooks, not backfill — so the install universe is still under-counted historically.
- **Fix:** drive event fetch from the install-event/store universe (or a store list) rather than only existing subscriptions, so non-converting installs are captured. Then the funnel's Installs stage reflects reality. Same event pipeline also feeds the Installs report and Uninstall Context.
- **Resolved (ADR-046):** the `DEV LIMIT` 10-subscription caps in the event + status processors, and the 1-month/12-month sync windows (now full history from `SyncHistoryStart`).
